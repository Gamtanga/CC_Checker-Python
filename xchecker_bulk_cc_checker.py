...
def main():
    if os.path.exists(ccFile):
        with open(ccFile) as f:
            writeFileOutput("Running Xchecker script...", outputFile)
            writeFileOutput("Output file results: {}".format(outputFile), outputFile)
            writeFileOutput("Any issue, contact me at:", outputFile)
            writeFileOutput("Telegram: @Xchecker_cc_Adm", outputFile)
            writeFileOutput("Email: xchecker_contact@protonmail.com", outputFile)
            writeFileOutput("Web: https://www.xchecker.cc/", outputFile)
